...
> - **Committed photos removed** — the 60 files (~126 MB) in `backend/app/uploads/` are untracked and ignored by git and Vercel. History rewritten 2026-09-25 (`git filter-repo --path backend/app/uploads --invert-paths`, every branch force-pushed; a clone went from 153 MB to 29 MB). Existing clones must be re-cloned. ⚠️ GitHub still serves the old commits through the closed PRs' `refs/pull/*` refs and its caches until GitHub Support purges them.
