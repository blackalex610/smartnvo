      ...
      <div className={`w-full ${maxWidthClassName} mx-auto px-4 sm:px-6`}>
        <div className="flex justify-between h-14 items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            {/* ── Phone pairing — always visible inline ── */}
            {canUseMobileConnect && (
              <PhonePairingInline
                roomCode={roomCode}
                isPaired={isPaired}
                ...
                onMount={() => void ensureRoom()}
                onRegenerate={() => void regenerateRoom()}
              />
            )}
...
// ── Inline always-visible phone pairing widget ──────────────────────────────
interface PhonePairingInlineProps {
  roomCode: string;
  isPaired: boolean;
  status: string;
  deviceName: string;
...
const PhonePairingInline: React.FC<PhonePairingInlineProps> = ({
  roomCode, isPaired, status, deviceName, onMount, onRegenerate,
}) => {
  useEffect(() => { onMount(); }, []);
    ...
    <div className="hidden md:flex items-center gap-1.5 rounded-lg border border-[#b8ddd0] bg-[#e8f8f0] px-2.5 py-1.5">
      ...
      {roomCode ? (
        ...
        <span className="text-[10px] text-slate-400 italic">
          {status === 'connecting' ? 'Зареждане…' : 'Грешка'}
        </span>
      )}
      <button
        type="button"
